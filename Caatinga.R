...
df <- read.csv("Caatinga.csv")
df$species <- factor(df$species)
index <- df %>% group_by(species) %>%
  summarise(n=n()) %>% filter(n>30)
...
ggplot() +
  ...
  labs(title= "Catimbau National Park Caatinga Brazil",
...
for(i in 1:length(spec)){
  ...
  ggsave(filename = paste("C:/Users/skly5321/Downloads/ChapterOne/ChapterOne/Plots/Caatinga_All_Plots/",spec[i],"_plot.png",sep=""),device = "png")
}
